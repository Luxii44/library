...
// auto run
func init() {
	system.RegisterInit(initOrderApi, &initAccount{})
}

func (i initAccount) InitializerName() string {
	return sysModel.SysApi{}.TableName()
}

func (i *initAccount) MigrateTable(ctx context.Context) (context.Context, error) {
...
func (i *initAccount) TableCreated(ctx context.Context) bool {
	db, ok := ctx.Value("db").(*gorm.DB)
	if !ok {
		return false
	}
...
func (i *initAccount) InitializeData(ctx context.Context) (context.Context, error) {
	db, ok := ctx.Value("db").(*gorm.DB)
	if !ok {
		return ctx, system.ErrMissingDBContext
	}
...
func (i *initAccount) DataInserted(ctx context.Context) bool {
	db, ok := ctx.Value("db").(*gorm.DB)
	if !ok {
		return false
	}
